...
csv_file = np.genfromtxt(
    fileD, delimiter=",", dtype=float, encoding="utf-8-sig"
...
print(
    "Would you like to plot a specific speed? (y/n)"
)  # ask user if they would like to plot a specific speed
ss = input()
if ss == "n":
    print("")
else:
    print(
        "Please enter the speed curve required (%)"
    )  # custom speed curve plotting - must be integer
...
coeff = np.polyfit(
    x=flow, y=head, deg=2
)  # this find the equation of the line - 2nd order polynomial
...
print(
    "Would you like to determine what speed relates to a specific flow and head? (y/n)"
)
...
if hh == "y":
    ...
    nd = (nb * qd) * (
        (-a1 + ((a1 ** 2) + (4 * a0 * ((pd / (qd ** 2)) - a2))) ** 0.5) / (2 * a0)
    )
    print(
        "Your desired head and flow will occur at a pump speed of approximately %s percent"
        % str(int(nd))
    )
